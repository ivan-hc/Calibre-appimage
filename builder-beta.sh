...
DOWNLOAD_PAGE=$(curl -Ls https://download.calibre-ebook.com/betas/ | tr '">< ' '\n' | grep -i x86_64 | tail -1)
DOWNLOAD_URL="https://download.calibre-ebook.com/betas/$DOWNLOAD_PAGE"
...
ARCH=x86_64 _appimagetool --comp zstd --mksquashfs-opt -Xcompression-level --mksquashfs-opt 20 \
	-u "$UPINFO" \
	./"$APP".AppDir "$APPNAME"-beta_"$VERSION"-x86_64.AppImage
